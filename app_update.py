...
def property_detail(property_id):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            # Get work records
            cur.execute("""
                SELECT w.*, i.image_path 
                FROM propintel.work w
                LEFT JOIN propintel.property_images i ON w.property_id = i.property_id 
                    AND i.image_type = 'work' 
                    AND i.description LIKE '%' || w.work_description || '%'
            ...
            # Get expense records with receipt images
            cur.execute("""
                SELECT e.*, i.image_path 
                FROM propintel.money_out e
                LEFT JOIN propintel.property_images i ON e.property_id = i.property_id 
                    AND i.image_type = 'receipt' 
                    AND i.description LIKE '%' || e.expense_details || '%'
            ...
            cur.execute("""
                ...
                WHERE property_id = %s
            """, (property_id,))
            expense_categories = cur.fetchone()
            ...
            map_lat = property_data['latitude'] if property_data['latitude'] else 40.7128
            map_lng = property_data['longitude'] if property_data['longitude'] else -74.0060
